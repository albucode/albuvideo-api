  ...
  def perform
    ...
    User.all.each do |user|
      invoice = Invoice.create!(start_date: beginning_of_last_month, end_date: end_of_last_month, user_id: user.id,
                                status: 0)
      InvoiceItem.create!(service_id: service.id, quantity: previous_month_streaming_time(user.id), invoice_id: invoice.id, user_id: user.id,
                          price: service.price)
    end
  end

  def previous_month_streaming_time(id)
    beginning_of_last_month = Time.zone.now.last_month.beginning_of_month
    end_of_last_month = Time.zone.now.last_month.end_of_month
    VideoStreamEvent.where(user_id: id, created_at: beginning_of_last_month..end_of_last_month).sum(:duration)
  end
end
